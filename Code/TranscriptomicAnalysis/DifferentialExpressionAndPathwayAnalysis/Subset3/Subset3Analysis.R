...
samples <- read.table(file.path("Subset3Samples.txt"), header=TRUE)
...
write.csv(upReg, "Subset3_up_regulated_genes.csv")
...
# Adds the retrieved attributes to the dataframe containing the
# downregulated genes
write.csv(downReg, "Subset3_down_regulated_genes.csvs")
...
enrichment_chart(result_df = allPRF, top_terms = 15)

# Saves the data workspace
save.image("Subset3-image.Rdata")
